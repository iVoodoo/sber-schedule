...
function handleClick(e) {
    ...
    const lessons = (lesson) =>{
        ...
        if(get_schedule_json(document.getElementById('group_num').value)['grid'][day_num][lesson].length == 0){
            return (null);
        }
        else{
            let lesson_index = 0;
            let les = get_schedule_json(document.getElementById('group_num').value)['grid'][day_num][lesson];
            ...
            let loc = "";
            if(les[lesson_index]["location"].length > 0 && les[lesson_index]["shortRooms"].length > 0){
                loc = les[lesson_index]["location"] + ', ' +les[lesson_index]["shortRooms"][0];
            }
            if(les[lesson_index]["location"].length == 0 && les[lesson_index]["shortRooms"].length > 0){
                ...
                <div>
                    <Card style={{ width: "75vw", marginBottom: '1rem' }}>
                        <CardBody>
                            <CardContent>
                                <TextBox>
                                    <Badge style={{ marginRight: '1.25rem' }} text={time} size="l" />
                                    <Badge text={loc} size="l" view='secondary' />
                                </TextBox>
                                <TextBox>
                                    <TextBoxBigTitle>{les[lesson_index]["sbj"]}</TextBoxBigTitle>
                                    <TextBoxSubTitle>{les[lesson_index]["teacher"]}</TextBoxSubTitle>
                                </TextBox>
                            </CardContent>
                        </CardBody>
                    </Card>
    ...
    const elem = () => {
        ...
        day_num = 3
        if( day_num == 7 ||
            (get_schedule_json(document.getElementById('group_num').value)['grid'][day_num][1].length == 0 &&
            get_schedule_json(document.getElementById('group_num').value)['grid'][day_num][2].length == 0 &&
            get_schedule_json(document.getElementById('group_num').value)['grid'][day_num][3].length == 0 &&
            get_schedule_json(document.getElementById('group_num').value)['grid'][day_num][4].length == 0 &&
            get_schedule_json(document.getElementById('group_num').value)['grid'][day_num][5].length == 0 &&
            get_schedule_json(document.getElementById('group_num').value)['grid'][day_num][6].length == 0 &&
            get_schedule_json(document.getElementById('group_num').value)['grid'][day_num][7].length == 0)){
                return (
                    <div>
                        <Row style={{ marginBottom: '1rem', justifyContent: 'center' }}>
                            <TextBox>
    ...
    return (
        <div>
            <form action="" onSubmit={handleClick} method="PATH">
                <Row style={{ marginBottom: '1rem', justifyContent: 'center' }}>
                    <TextField id="group_num" name="group_num" size="l" placeholder='Введите номер группы' defaultValue="201-361" />
                </Row>
                <Row style={{ marginBottom: '1rem', justifyContent: 'center' }}>
